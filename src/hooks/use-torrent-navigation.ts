...
export function useTorrentNavigation(options: UseTorrentNavigationOptions = {}) {
    ...
    const selectNext = useCallback(() => {
        ...
        setFocusedTorrentIndex(prevIndex => {
            const nextIndex = prevIndex < currentFilteredTorrents.length - 1
                ? prevIndex + 1
                : 0; // Wrap to first
    ...
    const selectPrevious = useCallback(() => {
        ...
        setFocusedTorrentIndex(prevIndex => {
            const newIndex = prevIndex > 0
                ? prevIndex - 1
                : currentFilteredTorrents.length - 1; // Wrap to last
    ...
    const closeDetails = useCallback(() => {
        ...
        const isOnDetailPage = location.pathname.includes('/torrents/') &&
    ...
    useEffect(() => {
        const currentFilteredTorrents = getFilteredTorrents();
        if (currentFilteredTorrents.length > 0 && focusedTorrentIndex === -1 && selectedTorrents.length === 0) {
