...
export function useTorrentSelection(options: UseTorrentSelectionOptions = {}) {
    ...
    const selectAll = useCallback(() => {
        const { selectedTorrents, selectAllTorrents, clearSelection, getFilteredTorrents } = useTorrentStore.getState();
    ...
    const selectAllDownloading = useCallback(() => {
        ...
        const downloadingTorrents = filteredTorrents.filter(t =>
            ['downloading', 'metaDL', 'stalledDL', 'queuedDL', 'checkingDL', 'forcedDL'].includes(t.state)
        );
