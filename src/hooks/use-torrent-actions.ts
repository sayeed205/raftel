...
import { createPlatformShortcut, useKeyboardShortcuts, type KeyboardShortcut } from './use-keyboard-shortcuts';
...
export function useTorrentActions(options: UseTorrentActionsOptions = {}) {
    ...
    const togglePauseResume = useCallback(async () => {
        const { selectedTorrents, pauseTorrents, resumeTorrents, getTorrentByHash } = useTorrentStore.getState();
    ...
    const deleteSelectedTorrents = useCallback(async () => {
        const { selectedTorrents, deleteTorrents, getTorrentByHash } = useTorrentStore.getState();
        if (selectedTorrents.length === 0) return;
        ...
        const displayNames = torrentNames.length > 3
            ? `${torrentNames.join(', ')} and ${selectedTorrents.length - 3} more`
            : torrentNames.join(', ');

        const confirmMessage = selectedTorrents.length === 1
