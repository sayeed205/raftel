...
export function SettingsValidationDisplay({ errors }: SettingsValidationDisplayProps) {
        ...
        <div className='flex items-center justify-between'>
          <div className='flex items-center gap-2'>
            ...
            <Badge variant='destructive' className='text-xs'>
              {errors.length} error{errors.length !== 1 ? 's' : ''} in {fieldCount} field
              {fieldCount !== 1 ? 's' : ''}
            </Badge>
          </div>
          <Button variant='ghost' size='sm' onClick={clearValidationErrors} className='h-6 w-6 p-0'>
            <X className='h-4 w-4' />
          </Button>
        </div>
            ...
            <div className='space-y-2'>
              {Object.entries(errorsByField).map(([field, fieldErrors]) => (
                <div key={field} className='border-destructive border-l-2 pl-3'>
                  <div className='text-sm font-medium capitalize'>{field.replace(/_/g, ' ')}</div>
